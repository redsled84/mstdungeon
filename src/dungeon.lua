...
function Dungeon:consts()
	...
	self.MapWidth, self.MapHeight = 100, 65

	self.RoomPlacementAttempts = 0
	self.MaxRoomPlacementAttempts = 400

	self.NumRooms = 0
	self.MaxNumRooms = 14
	self.MinNumChests, self.MaxNumChests = 1, 3
	self.MinRoomSize, self.MaxRoomSize = 8, 14
	self.Failed = false
...
function Dungeon:getMapPositionFromCoordinates(x, y)
	return math.ceil(x / tileSize), math.ceil(x / tileSize)
end
